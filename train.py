...
import matplotlib
matplotlib.use('Agg')  # need this line for linux
...
def train(
    ...
    ### Generate the second class of GAN data here ###
    # stage_class=1 ## the labels of N1
    # generate_x=gan_x(train_x,train_y,stage_class,1);
    # np.savez(f"{gen_dir}/fold{fold_idx}",generate_x)
    # # Data=np.load(f"{gen_dir}/fold{fold_idx}.npz")
    # # generate_x=Data['arr_0'];
    # for j in range(len(train_x)):
    #     train_x[j]=np.vstack((train_x[j],generate_x[j]));
    #     train_y[j]=np.hstack((train_y[j],np.repeat(stage_class,generate_x[j].shape[0])));
    #     assert len(train_x[j]) == len(train_y[j])
    ...
    #### Generate GAN validation data here ####
    # Generate_x=gan_x(valid_x,valid_y,stage_class,0);
    # for j in range(len(valid_x)):
    #     valid_x[j]=np.vstack((valid_x[j],Generate_x[j]));
    #     valid_y[j]=np.hstack((valid_y[j],np.repeat(stage_class,Generate_x[j].shape[0])));
    #     assert len(valid_x[j]) == len(valid_y[j])
    ...
    ##we want to select 5 models to ensemble learning
    update_epoch = 0
    ##best model
    best_acc = np.arange(0,0.1,0.01)
    best_mf = np.arange(0,0.1,0.01)
    ...
    for epoch in range(model.get_current_epoch(), config["n_epochs"]):
        ...
        # Check best model1
        if best_acc[0] < valid_outs["test/accuracy"] and \
           best_mf[0] <= valid_outs["test/f1_score"]:
            best_acc[0] = valid_outs["test/accuracy"]
            best_mf[0] = valid_outs["test/f1_score"]
            update_epoch = epoch+1
            model.save_best_checkpoint(name="best_model1",idx=1)
        
        # Check best model2
        for j in range(1,10,1):
            if best_acc[j] < valid_outs["test/accuracy"] and \
               best_acc[j-1] > valid_outs["test/accuracy"] and \
               best_mf[j] <= valid_outs["test/f1_score"] and \
               best_mf[j-1] > valid_outs["test/f1_score"]:
                best_acc[j] = valid_outs["test/accuracy"]
                best_mf[j] = valid_outs["test/f1_score"]
                update_epoch = epoch+1
                model.save_best_checkpoint(name=f"best_model{j+1}",idx=j+1)
