...
def predict(
    ...
    for fold_idx in range(config["n_folds"]):
        ...
        for jj in range(n_model):
            ...
            logger.info("")
            if (model_idx==1):
                S_preds1.extend(s_preds)
            elif (model_idx==2):
                S_preds2.extend(s_preds)
            elif (model_idx==3):
                S_preds3.extend(s_preds)
            elif (model_idx==4):
                S_preds4.extend(s_preds)
            elif (model_idx==5):
                S_preds5.extend(s_preds)
            elif (model_idx==6):
                S_preds6.extend(s_preds)
            elif (model_idx==7):
                S_preds7.extend(s_preds)
            elif (model_idx==8):
                S_preds8.extend(s_preds)
            elif (model_idx==9):
                S_preds9.extend(s_preds)
            elif (model_idx==10):
                S_preds10.extend(s_preds)

                
        S_preds.extend(stats.mode((S_preds1,S_preds2,S_preds3,S_preds4,S_preds5,
                                   S_preds6,S_preds7,S_preds8,S_preds9,S_preds10))[0][0])
        trues.extend(s_trues)
        preds.extend(S_preds)
