...
class TinySleepNet(object):
    ...
    def __init__(
        ...
        # Saver for storing variables  ##11.30 revised this 
        self.saver = tf.train.Saver(tf.global_variables(), max_to_keep=1)
        ## self.best_saver = tf.train.Saver(tf.global_variables(), max_to_keep=1)
        for j in range(10):
            jj=j+1
            exec('self.best_saver%d = tf.train.Saver(tf.global_variables(), max_to_keep=1)'%jj)
        ...
        is_restore = False
        if use_best:
            self.best_ckpt_path=os.path.join(self.output_dir, f"best_ckpt{model_index}")            
    ...
    def evaluate(self, minibatches,model_index=1):
        start = timeit.default_timer()
        losses = []
        preds = []
        trues = []
    ...
    def save_best_checkpoint(self, name, idx):
        path = eval(f"self.best_saver{idx}").save(
            self.sess,
            os.path.join(os.path.join(self.output_dir, f"best_ckpt{idx}"), "{}.ckpt".format(name)),
            global_step=self.global_step
        )
